...
impl GameState {
        ...
        for_each_tile_on_screen(self.camera_offset, rt.size(), |tp, sp| {
            ...
            if !tile.mid.empty() {
                ...
                let rect = if !def.neigh_aware {
                    ...
                    let left = self.world.tile_at_mut(tp.x_off(-1)).mid == tile.mid;
                    let right = self.world.tile_at_mut(tp.x_off(1)).mid == tile.mid;
                    let above = self.world.tile_at_mut(tp.y_off(-1)).mid == tile.mid;
                    let below = self.world.tile_at_mut(tp.y_off(1)).mid == tile.mid;
                    adjust_blend_rect(&mut rect, left, right, above, below);
                    rect
                };
                if let Some(state) = self.transient_block_state.get(&tp) {
    ...
    pub fn draw_entities(&mut self, rt: &mut RenderTexture, res: &Res) {
        let mut s = Sprite::with_texture(&res.atlas.tex);
        s.set_origin((16., 16.));
        for drop in &self.item_drops {
            let pos = drop.s2dc_en.en.pos;
            let item_def = &self.itemdb.get(drop.id).unwrap();
            imm_dbg!(&item_def.name);
            imm_dbg!(&item_def.graphic_name);
            s.set_position((
                (pos.x - self.camera_offset.x as i32) as f32,
                (pos.y - self.camera_offset.y as i32) as f32,
            ));
    ...
    pub fn draw_ui(&mut self, rt: &mut RenderTexture, res: &Res, ui_dims: Vector2f) {
        ...
        if let Some(def) = self
            .itemdb
            .get(self.inventory.slots[self.selected_inv_slot].id)
        {
        ...
        for (i, slot) in self.inventory.slots.iter().enumerate() {
            ...
            let Some(item_def) = &self.itemdb.get(slot.id) else {
                continue;
            };
